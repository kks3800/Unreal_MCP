...
// Live-edit redirection: route mutations to the editor's preview material when open
#include "Editor.h"
...
UMaterial* FUnrealMCPMaterialCommands::LoadMaterial(const FString& MaterialName, FString& OutPath)
{
	...
	if (MaterialName.StartsWith(TEXT("/")))
	{
		...
		OriginalMaterial = Cast<UMaterial>(UEditorAssetLibrary::LoadAsset(MaterialName));
	}
	else
	{
	...
	// Live-edit redirection: when the Material Editor is already open for this asset,
	// it edits a transient duplicate (FMaterialEditor::Material is a UPreviewMaterial copy
	// of OriginalMaterial — see Engine/MaterialEditor.cpp:557). Mutating OriginalMaterial
	// would leave the duplicate stale, so the editor wouldn't show changes until close+reopen.
	// Routing edits to the duplicate makes them appear live in the open graph view.
	// Persistence works as normal: when the user saves in the editor, FMaterialEditor copies
	// the duplicate's state back to OriginalMaterial.
	if (OriginalMaterial && GEditor)
	{
		if (UAssetEditorSubsystem* Subsystem = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>())
		{
			if (IAssetEditorInstance* Instance = Subsystem->FindEditorForAsset(OriginalMaterial, /*bFocusIfOpen=*/false))
			{
				IMaterialEditor* MatEditor = static_cast<IMaterialEditor*>(Instance);
				if (UMaterial* PreviewMaterial = Cast<UMaterial>(MatEditor->GetMaterialInterface()))
				{
...
#else
	UClass* FoundClass = FindObject<UClass>(nullptr, *ClassName, /*bExactClass=*/true);
#endif
...
#else
	FoundClass = FindObject<UClass>(nullptr, *FullClassName, /*bExactClass=*/false);
#endif
	...
	if (Params->TryGetArrayField(TEXT("nodes"), NodesArray))
	{
		for (int32 i = 0; i < NodesArray->Num(); i++)
		{
			...
			else if (UMaterialExpressionCustom* CustomExpr = Cast<UMaterialExpressionCustom>(Expr))
			{
				...
				if ((*NodeObj)->TryGetStringField(TEXT("output_type"), OutputTypeStr))
				{
					if (OutputTypeStr.Contains(TEXT("Float1")) || OutputTypeStr.Equals(TEXT("float"), ESearchCase::IgnoreCase))
					{
						CustomExpr->OutputType = CMOT_Float1;
					}
					else if (OutputTypeStr.Contains(TEXT("Float2")) || OutputTypeStr.Equals(TEXT("float2"), ESearchCase::IgnoreCase))
					{
						CustomExpr->OutputType = CMOT_Float2;
					}
					else if (OutputTypeStr.Contains(TEXT("Float3")) || OutputTypeStr.Equals(TEXT("float3"), ESearchCase::IgnoreCase))
					{
						CustomExpr->OutputType = CMOT_Float3;
					}
					else if (OutputTypeStr.Contains(TEXT("Float4")) || OutputTypeStr.Equals(TEXT("float4"), ESearchCase::IgnoreCase))
					{
			...
			else if (UMaterialExpressionComponentMask* MaskExpr = Cast<UMaterialExpressionComponentMask>(Expr))
			{
				bool bVal = false;
				if ((*NodeObj)->TryGetBoolField(TEXT("r"), bVal)) { MaskExpr->R = bVal; }
				if ((*NodeObj)->TryGetBoolField(TEXT("g"), bVal)) { MaskExpr->G = bVal; }
				if ((*NodeObj)->TryGetBoolField(TEXT("b"), bVal)) { MaskExpr->B = bVal; }
				if ((*NodeObj)->TryGetBoolField(TEXT("a"), bVal)) { MaskExpr->A = bVal; }
			}
...
TSharedPtr<FJsonObject> FUnrealMCPMaterialCommands::HandleCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
	...
	else if (CommandType == TEXT("compile_material_detailed"))
	{
		return HandleCompileMaterialDetailed(Params);
	}
...
TSharedPtr<FJsonObject> FUnrealMCPMaterialCommands::HandleAddMaterialNode(const TSharedPtr<FJsonObject>& Params)
{
	...
	// Handle MaterialFunctionCall — assign target function and regenerate pins.
	// SetMaterialFunction calls UpdateFromFunctionResource which populates
	// FunctionInputs/FunctionOutputs so subsequent connect_material_nodes calls
	// can resolve named pins.
	if (UMaterialExpressionMaterialFunctionCall* FuncCall = Cast<UMaterialExpressionMaterialFunctionCall>(NewExpression))
	{
		FString FuncPath;
		bool bHaveFuncPath = Params->TryGetStringField(TEXT("function_path"), FuncPath);
		if (!bHaveFuncPath)
		{
			// Also accept it nested under "properties": { "MaterialFunction": "/Game/..." }
			const TSharedPtr<FJsonObject>* PropsObj = nullptr;
			if (Params->TryGetObjectField(TEXT("properties"), PropsObj))
			{
				bHaveFuncPath = (*PropsObj)->TryGetStringField(TEXT("MaterialFunction"), FuncPath);
			}
		}
		if (bHaveFuncPath)
		{
			FString LoadedFuncPath;
			UMaterialFunction* TargetFunc = LoadMaterialFunction(FuncPath, LoadedFuncPath);
			if (TargetFunc)
			{
				FuncCall->SetMaterialFunction(TargetFunc);
			}
		}
	}
...
namespace
{
	// Apply a JSON value to a leaf FProperty. Returns true if the property type
	// was understood and assignment succeeded.
	bool ApplyJsonValueToLeafProperty(FProperty* Prop, void* ValuePtr, const TSharedPtr<FJsonValue>& Value)
	{
		if (!Prop || !ValuePtr || !Value.IsValid()) { return false; }
		...
		{
			double N;
			if (Value->TryGetNumber(N)) { FloatProp->SetPropertyValue(ValuePtr, static_cast<float>(N)); return true; }
		}
		else if (FDoubleProperty* DoubleProp = CastField<FDoubleProperty>(Prop))
		{
			double N;
			if (Value->TryGetNumber(N)) { DoubleProp->SetPropertyValue(ValuePtr, N); return true; }
		}
		else if (FIntProperty* IntProp = CastField<FIntProperty>(Prop))
		{
			double N;
			if (Value->TryGetNumber(N)) { IntProp->SetPropertyValue(ValuePtr, static_cast<int32>(N)); return true; }
		}
		else if (FStrProperty* StrProp = CastField<FStrProperty>(Prop))
		{
			FString S;
			if (Value->TryGetString(S)) { StrProp->SetPropertyValue(ValuePtr, S); return true; }
		}
		else if (FNameProperty* NameProp = CastField<FNameProperty>(Prop))
		{
			FString S;
			if (Value->TryGetString(S)) { NameProp->SetPropertyValue(ValuePtr, FName(*S)); return true; }
		}
		else if (FEnumProperty* EnumProp = CastField<FEnumProperty>(Prop))
		{
			...
			{
				int64 V = EnumProp->GetEnum()->GetValueByNameString(S);
				if (V != INDEX_NONE) { EnumProp->GetUnderlyingProperty()->SetIntPropertyValue(ValuePtr, V); return true; }
			}
			else if (Value->TryGetNumber(N))
			{
				EnumProp->GetUnderlyingProperty()->SetIntPropertyValue(ValuePtr, static_cast<int64>(N));
				return true;
			}
		}
		...
		{
			FString S; double N;
			if (ByteProp->Enum && Value->TryGetString(S))
			{
				int64 V = ByteProp->Enum->GetValueByNameString(S);
				if (V != INDEX_NONE) { ByteProp->SetPropertyValue(ValuePtr, static_cast<uint8>(V)); return true; }
			}
			else if (Value->TryGetNumber(N))
			{
		...
		{
			// Null / empty-string clears the reference (used to disconnect
			// FExpressionInput.Expression pins via path assignment).
			...
			if (Value->TryGetString(S))
			{
				...
				UObject* LoadedAsset = UEditorAssetLibrary::LoadAsset(S);
				if (LoadedAsset && LoadedAsset->IsA(ObjProp->PropertyClass))
				{
					ObjProp->SetObjectPropertyValue(ValuePtr, LoadedAsset);
					return true;
				}
			}
		}
	...
	// Walk a dotted/bracketed path like "Layers[0].BlendType" against a UStruct
	// container, returning the leaf FProperty + raw value pointer on success.
	// Supports descending into FStructProperty and indexing into FArrayProperty.
	bool ResolvePropertyPath(UStruct* OuterStruct, void* OuterPtr, const FString& Path,
		FProperty*& OutLeafProp, void*& OutLeafPtr, FString& OutError)
	{
		...
		UStruct* CurStruct = OuterStruct;
		void* CurPtr = OuterPtr;
		FProperty* CurProp = nullptr;

		for (int32 i = 0; i < Segments.Num(); ++i)
		{
			const FString& Seg = Segments[i];
			FString FieldName = Seg;
			int32 Index = INDEX_NONE;
			int32 BracketStart;
			if (Seg.FindChar('[', BracketStart))
			{
				int32 BracketEnd;
				if (!Seg.FindChar(']', BracketEnd) || BracketEnd <= BracketStart + 1)
				{
					OutError = FString::Printf(TEXT("Malformed array index in segment '%s'"), *Seg);
					return false;
				}
				FString IndexStr = Seg.Mid(BracketStart + 1, BracketEnd - BracketStart - 1);
				Index = FCString::Atoi(*IndexStr);
				FieldName = Seg.Left(BracketStart);
			}

			if (!CurStruct)
			{
				OutError = FString::Printf(TEXT("Path '%s' descended past a non-struct value before '%s'"), *Path, *FieldName);
				return false;
			}

			CurProp = CurStruct->FindPropertyByName(FName(*FieldName));
			if (!CurProp)
			{
				OutError = FString::Printf(TEXT("Property not found: %s (in %s)"), *FieldName, *CurStruct->GetName());
				return false;
			}
			CurPtr = CurProp->ContainerPtrToValuePtr<void>(CurPtr);

			if (Index != INDEX_NONE)
			{
				FArrayProperty* ArrProp = CastField<FArrayProperty>(CurProp);
				if (!ArrProp)
				{
					OutError = FString::Printf(TEXT("Property '%s' is not an array (segment '%s')"), *FieldName, *Seg);
					return false;
				}
				FScriptArrayHelper ArrayHelper(ArrProp, CurPtr);
				if (Index < 0 || Index >= ArrayHelper.Num())
				{
					OutError = FString::Printf(TEXT("Index %d out of bounds (size %d) for '%s'"), Index, ArrayHelper.Num(), *FieldName);
					return false;
				}
				CurPtr = ArrayHelper.GetRawPtr(Index);
			...
			if (i + 1 < Segments.Num())
			{
				...
				if (!StructProp)
				{
					OutError = FString::Printf(TEXT("Property '%s' is not a struct, cannot descend further"), *FieldName);
					return false;
				}
...
TSharedPtr<FJsonObject> FUnrealMCPMaterialCommands::HandleSetMaterialNodeProperty(const TSharedPtr<FJsonObject>& Params)
{
	...
	// Texture SamplerSource (FromAsset, Wrap, Clamp) — affects hardware sampler sharing
	FString SamplerSourceStr;
	if (Params->TryGetStringField(TEXT("sampler_source"), SamplerSourceStr))
	{
		if (UMaterialExpressionTextureSample* SampleExpr = Cast<UMaterialExpressionTextureSample>(Expression))
		{
			if (SamplerSourceStr.Equals(TEXT("FromAsset"), ESearchCase::IgnoreCase) ||
				SamplerSourceStr.Equals(TEXT("FromTextureAsset"), ESearchCase::IgnoreCase))
			{
				SampleExpr->SamplerSource = SSM_FromTextureAsset;
			}
			else if (SamplerSourceStr.Equals(TEXT("Wrap"), ESearchCase::IgnoreCase) ||
				SamplerSourceStr.Equals(TEXT("SharedWrap"), ESearchCase::IgnoreCase))
			{
				SampleExpr->SamplerSource = SSM_Wrap_WorldGroupSettings;
			}
			else if (SamplerSourceStr.Equals(TEXT("Clamp"), ESearchCase::IgnoreCase) ||
				SamplerSourceStr.Equals(TEXT("SharedClamp"), ESearchCase::IgnoreCase))
			{
				SampleExpr->SamplerSource = SSM_Clamp_WorldGroupSettings;
			}
		}
	}

	// Generic UPROPERTY setter via reflection — set any property by name
	...
	if (Params->TryGetObjectField(TEXT("properties"), PropertiesObj))
	{
		...
		// Handle MaterialFunctionCall.MaterialFunction specially — must call
		// SetMaterialFunction so UpdateFromFunctionResource regenerates pins.
		// Direct UPROPERTY assignment via reflection on TObjectPtr would skip
		// pin regeneration, leaving the call node stuck without inputs/outputs.
		if (UMaterialExpressionMaterialFunctionCall* FuncCall = Cast<UMaterialExpressionMaterialFunctionCall>(Expression))
		{
			FString FuncPath;
			if ((*PropertiesObj)->TryGetStringField(TEXT("MaterialFunction"), FuncPath))
			{
				FString LoadedFuncPath;
				UMaterialFunction* TargetFunc = LoadMaterialFunction(FuncPath, LoadedFuncPath);
				if (TargetFunc)
				{
					FuncCall->SetMaterialFunction(TargetFunc);
					SetProperties.Add(TEXT("MaterialFunction"));
				}
			}
		}
		...
		{
			// Skip MaterialFunction here — already handled above with proper pin refresh.
			if (Pair.Key == TEXT("MaterialFunction") &&
				Cast<UMaterialExpressionMaterialFunctionCall>(Expression))
			{
				continue;
			}

			// Path-style keys ("Layers[0].BlendType") drill into TArray/struct
			// members via reflection. Plain keys still go through the legacy
			// direct lookup below.
			if (Pair.Key.Contains(TEXT(".")) || Pair.Key.Contains(TEXT("[")))
			{
				FProperty* LeafProp = nullptr;
				void* LeafPtr = nullptr;
				FString PathErr;
				if (ResolvePropertyPath(ExprClass, Expression, Pair.Key, LeafProp, LeafPtr, PathErr)
					&& ApplyJsonValueToLeafProperty(LeafProp, LeafPtr, Pair.Value))
				{
			...
			else if (FEnumProperty* EnumProp = CastField<FEnumProperty>(Prop))
			{
				...
				{
					int64 EnumValue = EnumProp->GetEnum()->GetValueByNameString(StrVal);
				...
				else if (Pair.Value->TryGetNumber(NumVal))
				{
			...
			else if (FByteProperty* ByteProp = CastField<FByteProperty>(Prop))
			{
				FString StrVal;
				double NumVal;
				if (ByteProp->Enum && Pair.Value->TryGetString(StrVal))
				{
			...
			{
				// Bind any UObject reference (e.g. CollectionParameter.Collection,
				// TextureSample.Texture) by asset path. Loads the asset and stores
				// the pointer if its class is compatible with the property class.
				FString AssetPath;
				if (Pair.Value->TryGetString(AssetPath))
				{
		...
		// CollectionParameter — resolve ParameterId from Collection + ParameterName.
		// Without this the node displays "None" / errors even with both fields set,
		// because the runtime looks parameters up by GUID, not by name.
		if (UMaterialExpressionCollectionParameter* CP = Cast<UMaterialExpressionCollectionParameter>(Expression))
		{
			if (CP->Collection && !CP->ParameterName.IsNone())
			{
				FGuid Found = CP->Collection->GetParameterId(CP->ParameterName);
				if (Found.IsValid())
				{
					CP->ParameterId = Found;
				}
			}
		}
		...
		if (SetProperties.Num() > 0)
		{
			...
			ResultObj->SetArrayField(TEXT("properties_set"), SetArr);
			if (Material) { Material->MarkPackageDirty(); }
			else if (Function) { Function->MarkPackageDirty(); }
			return ResultObj;
		}
...
TSharedPtr<FJsonObject> FUnrealMCPMaterialCommands::HandleConnectToMaterialOutput(const TSharedPtr<FJsonObject>& Params)
{
	...
	// Special case: when UseMaterialAttributes is enabled, the UMaterial exposes
	// a single FMaterialAttributesInput pin instead of the per-channel pins.
	// EMaterialProperty has no MP_MaterialAttributes value, so StringToMaterialProperty
	// falls through to MP_BaseColor which silently misroutes the connection.
	// Detect this case and wire directly to UMaterialEditorOnlyData::MaterialAttributes.
	if (MaterialProperty.Equals(TEXT("MaterialAttributes"), ESearchCase::IgnoreCase))
	{
		UMaterialEditorOnlyData* EdOnly = Material->GetEditorOnlyData();
		if (!EdOnly)
		{
			return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Material has no editor-only data"));
		}

		// Also clear any previous per-channel wiring that may have been made when
		// MaterialAttributes was off (otherwise stale BaseColor/Roughness/etc connections
		// stay and the editor warns about them).
		EdOnly->BaseColor.Expression = nullptr;
		EdOnly->Metallic.Expression = nullptr;
		EdOnly->Specular.Expression = nullptr;
		EdOnly->Roughness.Expression = nullptr;
		EdOnly->Normal.Expression = nullptr;
		EdOnly->EmissiveColor.Expression = nullptr;
		EdOnly->Opacity.Expression = nullptr;
		EdOnly->OpacityMask.Expression = nullptr;
		EdOnly->AmbientOcclusion.Expression = nullptr;

		// Resolve output index from the FromOutput name. For MaterialFunctionCall and
		// other multi-output nodes, the name must match one of the outputs; default 0.
		int32 OutputIndex = 0;
		if (!FromOutput.IsEmpty())
		{
			...
			{
				if (Outs[i].OutputName.ToString().Equals(FromOutput, ESearchCase::IgnoreCase))
				{
					OutputIndex = i;
					break;
				}
		...
		EdOnly->MaterialAttributes.Expression = FromExpression;
		EdOnly->MaterialAttributes.OutputIndex = OutputIndex;
		EdOnly->MaterialAttributes.Mask = 0;

		Material->MarkPackageDirty();

		TSharedPtr<FJsonObject> OkObj = MakeShared<FJsonObject>();
		OkObj->SetBoolField(TEXT("success"), true);
		return OkObj;
	}
...
TSharedPtr<FJsonObject> FUnrealMCPMaterialCommands::HandleCompileMaterialDetailed(const TSharedPtr<FJsonObject>& Params)
{
#if WITH_EDITOR
	FString MaterialName;
	if (!Params->TryGetStringField(TEXT("material_name"), MaterialName))
	...
	// Try loading as Material first, then as MaterialFunction
	FString AssetPath;
	UMaterial* Material = LoadMaterial(MaterialName, AssetPath);
	UMaterialFunction* Function = nullptr;
	if (!Material)
	{
		Function = LoadMaterialFunction(MaterialName, AssetPath);
		if (!Function)
		{
			return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Material or function not found: %s"), *MaterialName));
		}
	}

	// Recompile if it's a material
	if (Material)
	{
		UMaterialEditingLibrary::RecompileMaterial(Material);
	}
	...
	TArray<TSharedPtr<FJsonValue>> UnconnectedArr;
	...
	// 1) Gather shader compile errors from material resources (materials only)
	if (Material)
	{
		for (int32 QualityLevel = 0; QualityLevel < EMaterialQualityLevel::Num; ++QualityLevel)
		{
			...
			{
				// UE 5.7 takes EShaderPlatform, not ERHIFeatureLevel — translate via the
				// global table so we still iterate per feature level conceptually.
				const ERHIFeatureLevel::Type FL = static_cast<ERHIFeatureLevel::Type>(FeatureLevel);
				const EShaderPlatform Platform = GShaderPlatformForFeatureLevel[FL];
				if (Platform == SP_NumPlatforms) { continue; }
				const FMaterialResource* Resource = Material->GetMaterialResource(
					Platform,
					static_cast<EMaterialQualityLevel::Type>(QualityLevel));
				if (Resource)
				{
					TArray<FString> ResErrors;
					ResErrors = Resource->GetCompileErrors();
					for (const FString& Err : ResErrors)
					{
						TSharedPtr<FJsonObject> ErrObj = MakeShared<FJsonObject>();
						ErrObj->SetStringField(TEXT("error"), Err);
						ErrObj->SetStringField(TEXT("feature_level"),
							FeatureLevel == 0 ? TEXT("ES3_1") :
							FeatureLevel == 1 ? TEXT("SM5") :
							FeatureLevel == 2 ? TEXT("SM6") : TEXT("Unknown"));
	...
	// 2) Check each expression for unconnected inputs and errors
	UMaterialEditorOnlyData* EdOnly = Material ? Material->GetEditorOnlyData() : nullptr;

	// GetExpressions() returns TArrayView — copy to TArray for iteration
	...
	TSet<UMaterialExpression*> ConnectedExpressions;

	for (UMaterialExpression* Expr : AllExpressions)
	{
		if (!Expr) continue;

		FString NodeName = Expr->Desc.IsEmpty() ? Expr->GetName() : Expr->Desc;

		// Check Custom HLSL nodes for code issues
		if (UMaterialExpressionCustom* CustomExpr = Cast<UMaterialExpressionCustom>(Expr))
		{
			if (CustomExpr->Code.IsEmpty())
			{
				TSharedPtr<FJsonObject> ErrObj = MakeShared<FJsonObject>();
				ErrObj->SetStringField(TEXT("node"), NodeName);
				ErrObj->SetStringField(TEXT("error"), TEXT("Custom HLSL node has empty code"));
				ErrorsArr.Add(MakeShared<FJsonValueObject>(ErrObj));
			}
		}

		// Track which expressions are connected (as outputs to other nodes)
		const TArrayView<FExpressionInput*> Inputs = Expr->GetInputsView();
		for (FExpressionInput* Input : Inputs)
		{
			if (Input && Input->Expression)
			{
				ConnectedExpressions.Add(Input->Expression);
			}
		}

		// Report unconnected inputs for key node types
		bool bIsParameter = Expr->IsA<UMaterialExpressionParameter>() ||
		                     Expr->IsA<UMaterialExpressionTextureSampleParameter>();
		bool bIsConstant = Expr->IsA<UMaterialExpressionConstant>() ||
		                    Expr->IsA<UMaterialExpressionConstant3Vector>() ||
		                    Expr->IsA<UMaterialExpressionConstant4Vector>();
		bool bIsWorldData = Expr->IsA<UMaterialExpressionWorldPosition>() ||
		                     Expr->IsA<UMaterialExpressionPixelNormalWS>() ||
		                     Expr->IsA<UMaterialExpressionVertexNormalWS>() ||
		                     Expr->IsA<UMaterialExpressionCameraPositionWS>();
		bool bIsFuncIO = Expr->IsA<UMaterialExpressionFunctionInput>() ||
		                  Expr->IsA<UMaterialExpressionFunctionOutput>();

		// Skip nodes that have no required inputs
		if (bIsParameter || bIsConstant || bIsWorldData || bIsFuncIO)
		{
			continue;
		}
		...
		for (int32 i = 0; i < Inputs.Num(); ++i)
		{
			...
			if (Input && !Input->Expression)
			{
				FString InputName = Expr->GetInputName(i).ToString();
				if (InputName.IsEmpty()) InputName = FString::Printf(TEXT("Input_%d"), i);

				TSharedPtr<FJsonObject> UnconnObj = MakeShared<FJsonObject>();
				UnconnObj->SetStringField(TEXT("node"), NodeName);
				UnconnObj->SetStringField(TEXT("node_type"), Expr->GetClass()->GetName());
				UnconnObj->SetStringField(TEXT("unconnected_input"), InputName);
				UnconnObj->SetNumberField(TEXT("input_index"), i);
				UnconnectedArr.Add(MakeShared<FJsonValueObject>(UnconnObj));
			}
		}
	}

	// 3) Find orphaned nodes (no output connections, not wired to material/function output)
	TSet<UMaterialExpression*> OutputConnected;

	// For materials: check material output pins
	if (EdOnly)
	{
		auto CheckOutput = [&](FExpressionInput& Input)
		{
			if (Input.Expression)
			{
				OutputConnected.Add(Input.Expression);
			}
		};
		CheckOutput(EdOnly->BaseColor);
		CheckOutput(EdOnly->Metallic);
		CheckOutput(EdOnly->Specular);
		CheckOutput(EdOnly->Roughness);
		CheckOutput(EdOnly->Normal);
		CheckOutput(EdOnly->EmissiveColor);
		CheckOutput(EdOnly->Opacity);
		CheckOutput(EdOnly->OpacityMask);
		CheckOutput(EdOnly->AmbientOcclusion);
		CheckOutput(EdOnly->WorldPositionOffset);
		CheckOutput(EdOnly->SubsurfaceColor);
	}

	// For functions: FunctionOutput nodes are "terminal" — their inputs count as connected
	for (UMaterialExpression* Expr : AllExpressions)
	{
		if (UMaterialExpressionFunctionOutput* FuncOut = Cast<UMaterialExpressionFunctionOutput>(Expr))
		{
			OutputConnected.Add(Expr); // The output node itself is a terminal
			const TArrayView<FExpressionInput*> FuncOutInputs = FuncOut->GetInputsView();
			for (FExpressionInput* Input : FuncOutInputs)
			{
				if (Input && Input->Expression)
				{
					OutputConnected.Add(Input->Expression);
	...
	{
		if (!Expr) continue;
		if (ConnectedExpressions.Contains(Expr)) continue;
		if (OutputConnected.Contains(Expr)) continue;
		// FunctionInput nodes are source nodes, not orphaned
		if (Expr->IsA<UMaterialExpressionFunctionInput>()) continue;

		FString NodeName = Expr->Desc.IsEmpty() ? Expr->GetName() : Expr->Desc;
		TSharedPtr<FJsonObject> OrphanObj = MakeShared<FJsonObject>();
		OrphanObj->SetStringField(TEXT("node"), NodeName);
		OrphanObj->SetStringField(TEXT("node_type"), Expr->GetClass()->GetName());
		OrphanObj->SetStringField(TEXT("issue"), TEXT("Node output not connected to anything"));
		OrphanedArr.Add(MakeShared<FJsonValueObject>(OrphanObj));
	}

	// 4) Check which outputs are wired
	TArray<TSharedPtr<FJsonValue>> OutputStatusArr;

	// Material outputs
	if (EdOnly)
	{
		auto ReportOutput = [&](FExpressionInput& Input, const TCHAR* Name)
		{
			TSharedPtr<FJsonObject> OutObj = MakeShared<FJsonObject>();
			OutObj->SetStringField(TEXT("output"), Name);
			OutObj->SetBoolField(TEXT("connected"), Input.Expression != nullptr);
			if (Input.Expression)
			{
				FString FromName = Input.Expression->Desc.IsEmpty() ? Input.Expression->GetName() : Input.Expression->Desc;
				OutObj->SetStringField(TEXT("from_node"), FromName);
			}
			OutputStatusArr.Add(MakeShared<FJsonValueObject>(OutObj));
		};
		ReportOutput(EdOnly->BaseColor, TEXT("BaseColor"));
		ReportOutput(EdOnly->Metallic, TEXT("Metallic"));
		ReportOutput(EdOnly->Specular, TEXT("Specular"));
		ReportOutput(EdOnly->Roughness, TEXT("Roughness"));
		ReportOutput(EdOnly->Normal, TEXT("Normal"));
		ReportOutput(EdOnly->EmissiveColor, TEXT("EmissiveColor"));
		ReportOutput(EdOnly->Opacity, TEXT("Opacity"));
		ReportOutput(EdOnly->OpacityMask, TEXT("OpacityMask"));
		ReportOutput(EdOnly->AmbientOcclusion, TEXT("AmbientOcclusion"));
	}
	...
	if (Function)
	{
		for (UMaterialExpression* Expr : AllExpressions)
		{
			...
			{
				TSharedPtr<FJsonObject> OutObj = MakeShared<FJsonObject>();
				OutObj->SetStringField(TEXT("output"), FuncOut->OutputName.ToString());
				const TArrayView<FExpressionInput*> FuncOutInputs = FuncOut->GetInputsView();
				bool bConnected = FuncOutInputs.Num() > 0 && FuncOutInputs[0] && FuncOutInputs[0]->Expression != nullptr;
				OutObj->SetBoolField(TEXT("connected"), bConnected);
				if (bConnected)
				{
					FString FromName = FuncOutInputs[0]->Expression->Desc.IsEmpty() ? FuncOutInputs[0]->Expression->GetName() : FuncOutInputs[0]->Expression->Desc;
					OutObj->SetStringField(TEXT("from_node"), FromName);
				}
				OutputStatusArr.Add(MakeShared<FJsonValueObject>(OutObj));
			}
	...
	ResultObj->SetBoolField(TEXT("success"), true);
	ResultObj->SetNumberField(TEXT("error_count"), ErrorsArr.Num());
	ResultObj->SetNumberField(TEXT("warning_count"), WarningsArr.Num());
	ResultObj->SetNumberField(TEXT("unconnected_input_count"), UnconnectedArr.Num());
	ResultObj->SetNumberField(TEXT("orphaned_node_count"), OrphanedArr.Num());
	ResultObj->SetArrayField(TEXT("errors"), ErrorsArr);
	ResultObj->SetArrayField(TEXT("warnings"), WarningsArr);
	ResultObj->SetArrayField(TEXT("unconnected_inputs"), UnconnectedArr);
	ResultObj->SetArrayField(TEXT("orphaned_nodes"), OrphanedArr);
	ResultObj->SetArrayField(TEXT("material_outputs"), OutputStatusArr);
	ResultObj->SetNumberField(TEXT("total_nodes"), AllExpressions.Num());
...
// ============================================================================
// get_material_info — Full material introspection for AI learning
// ============================================================================

// Helper: build a name for an expression suitable for referencing in connections
static FString GetExprNodeName(UMaterialExpression* Expr)
{
	...
	if (UMaterialExpressionTextureSampleParameter* TP = Cast<UMaterialExpressionTextureSampleParameter>(Expr))
	{
		return TP->ParameterName.ToString();
	}
	if (!Expr->Desc.IsEmpty()) { return Expr->Desc; }
	return Expr->GetName();
}

// Helper: serialize one output pin reference (from an FExpressionInput)
static TSharedPtr<FJsonObject> SerializeInputConnection(FExpressionInput* Input, UMaterialExpression* OwnerExpr, int32 InputIndex)
{
	TSharedPtr<FJsonObject> Conn = MakeShared<FJsonObject>();
	if (Input && Input->Expression)
	{
		Conn->SetStringField(TEXT("from_node"), GetExprNodeName(Input->Expression));
		Conn->SetNumberField(TEXT("from_output_index"), Input->OutputIndex);
		if (Input->Expression->Outputs.IsValidIndex(Input->OutputIndex) &&
			!Input->Expression->Outputs[Input->OutputIndex].OutputName.IsNone())
		{
			Conn->SetStringField(TEXT("from_output_name"),
				Input->Expression->Outputs[Input->OutputIndex].OutputName.ToString());
		}
		Conn->SetStringField(TEXT("to_node"), GetExprNodeName(OwnerExpr));
		Conn->SetStringField(TEXT("to_input"), OwnerExpr->GetInputName(InputIndex).ToString());
		Conn->SetNumberField(TEXT("to_input_index"), InputIndex);
...
TSharedPtr<FJsonObject> FUnrealMCPMaterialCommands::HandleGetMaterialInfo(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!Params->TryGetStringField(TEXT("material_name"), MaterialName))
	{
		return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Missing 'material_name'"));
	}
	...
	UMaterial* Material = nullptr;
	{
		...
		if (!Material)
		{
			...
			{
				return FUnrealMCPCommonUtils::CreateErrorResponse(
					FString::Printf(TEXT("Material or function not found: %s"), *MaterialName));
			}
		}
	}
	...
	Result->SetStringField(TEXT("asset_type"), Material ? TEXT("Material") : TEXT("MaterialFunction"));
	Result->SetStringField(TEXT("asset_path"), Material
		? Material->GetPathName() : Function->GetPathName());

	// ---- MATERIAL SETTINGS (only for UMaterial, not functions) ----
	if (Material)
	{
		TSharedPtr<FJsonObject> Settings = MakeShared<FJsonObject>();

		// Domain & shading
		Settings->SetStringField(TEXT("MaterialDomain"),
			StaticEnum<EMaterialDomain>()->GetNameStringByValue(static_cast<int64>(Material->MaterialDomain)));
		Settings->SetStringField(TEXT("BlendMode"),
			StaticEnum<EBlendMode>()->GetNameStringByValue(static_cast<int64>(Material->BlendMode)));

		// Shading model — use the ShadingModels bitfield for multi-model support
		...
		{
			if (SMField.HasShadingModel(static_cast<EMaterialShadingModel>(i)))
			{
				SMArr.Add(MakeShared<FJsonValueString>(
					StaticEnum<EMaterialShadingModel>()->GetNameStringByValue(i)));
			}
		}
		Settings->SetArrayField(TEXT("ShadingModels"), SMArr);

		Settings->SetBoolField(TEXT("TwoSided"), Material->IsTwoSided());
		Settings->SetBoolField(TEXT("bUseMaterialAttributes"), Material->bUseMaterialAttributes != 0);
		Settings->SetNumberField(TEXT("OpacityMaskClipValue"), Material->OpacityMaskClipValue);
		Settings->SetBoolField(TEXT("bCastRayTracedShadows"), Material->bCastRayTracedShadows != 0);
		Settings->SetBoolField(TEXT("bFullyRough"), Material->bFullyRough != 0);
		Settings->SetBoolField(TEXT("bNormalCurvatureToRoughness"), Material->bNormalCurvatureToRoughness != 0);
		Settings->SetBoolField(TEXT("bIsThinSurface"), Material->bIsThinSurface != 0);
		Settings->SetBoolField(TEXT("bScreenSpaceReflections"), Material->bScreenSpaceReflections != 0);
		Settings->SetBoolField(TEXT("bContactShadows"), Material->bContactShadows != 0);
		Settings->SetBoolField(TEXT("Wireframe"), Material->Wireframe != 0);
		Settings->SetBoolField(TEXT("bIsSky"), Material->bIsSky != 0);

#if ENGINE_MINOR_VERSION >= 5
		Settings->SetBoolField(TEXT("bEnableTessellation"), Material->bEnableTessellation != 0);
#endif
		Settings->SetStringField(TEXT("TranslucencyLightingMode"),
			StaticEnum<ETranslucencyLightingMode>()->GetNameStringByValue(static_cast<int64>(Material->TranslucencyLightingMode)));

		// Usage flags
		TSharedPtr<FJsonObject> Usage = MakeShared<FJsonObject>();
		Usage->SetBoolField(TEXT("SkeletalMesh"), Material->GetUsageByFlag(MATUSAGE_SkeletalMesh));
		Usage->SetBoolField(TEXT("ParticleSprites"), Material->GetUsageByFlag(MATUSAGE_ParticleSprites));
		Usage->SetBoolField(TEXT("MeshParticles"), Material->GetUsageByFlag(MATUSAGE_MeshParticles));
		Usage->SetBoolField(TEXT("NiagaraSprites"), Material->GetUsageByFlag(MATUSAGE_NiagaraSprites));
		Usage->SetBoolField(TEXT("NiagaraMeshParticles"), Material->GetUsageByFlag(MATUSAGE_NiagaraMeshParticles));
		Usage->SetBoolField(TEXT("Nanite"), Material->GetUsageByFlag(MATUSAGE_Nanite));
		Settings->SetObjectField(TEXT("Usage"), Usage);

		Result->SetObjectField(TEXT("settings"), Settings);
	}

	// ---- FUNCTION BOUNDARY (only for MaterialFunction) ----
	if (Function)
	{
		TSharedPtr<FJsonObject> FuncInfo = MakeShared<FJsonObject>();
		FuncInfo->SetStringField(TEXT("Description"), Function->Description);
		...
		for (UMaterialExpression* Expr : Function->GetExpressions())
		{
			if (UMaterialExpressionFunctionInput* FI = Cast<UMaterialExpressionFunctionInput>(Expr))
			{
				...
				IO->SetStringField(TEXT("name"), FI->InputName.ToString());
				IO->SetStringField(TEXT("type"),
					StaticEnum<EFunctionInputType>()->GetNameStringByValue(static_cast<int64>(FI->InputType)));
				IO->SetStringField(TEXT("description"), FI->Description);
				IO->SetNumberField(TEXT("sort_priority"), FI->SortPriority);
				IO->SetBoolField(TEXT("use_preview_as_default"), FI->bUsePreviewValueAsDefault != 0);
			...
			{
				TSharedPtr<FJsonObject> IO = MakeShared<FJsonObject>();
				IO->SetStringField(TEXT("name"), FO->OutputName.ToString());
				IO->SetStringField(TEXT("description"), FO->Description);
				IO->SetNumberField(TEXT("sort_priority"), FO->SortPriority);
				OutputsArr.Add(MakeShared<FJsonValueObject>(IO));
			}
		}
		FuncInfo->SetArrayField(TEXT("inputs"), InputsArr);
		FuncInfo->SetArrayField(TEXT("outputs"), OutputsArr);
		Result->SetObjectField(TEXT("function_info"), FuncInfo);
	}
	...
	{
		if (UMaterialEditorOnlyData* EdOnly = Material->GetEditorOnlyData())
		{
			for (const TObjectPtr<UMaterialExpression>& Ptr : EdOnly->ExpressionCollection.Expressions)
			{
				if (Ptr) { Expressions.Add(Ptr.Get()); }
			}
		}
	...
	// Build lookup map for connection serialization
	TMap<UMaterialExpression*, FString> ExprNameMap;
	for (UMaterialExpression* Expr : Expressions)
	{
		ExprNameMap.Add(Expr, GetExprNodeName(Expr));
	}
	...
	for (UMaterialExpression* Expr : Expressions)
	{
		...
		FString NodeName = GetExprNodeName(Expr);
		Node->SetStringField(TEXT("name"), NodeName);
		...
		if (Expr->Outputs.Num() > 0)
		{
			...
			for (int32 i = 0; i < Expr->Outputs.Num(); ++i)
			{
				...
				OutObj->SetStringField(TEXT("name"), Expr->Outputs[i].OutputName.IsNone()
					? TEXT("") : Expr->Outputs[i].OutputName.ToString());
				OutsArr.Add(MakeShared<FJsonValueObject>(OutObj));
			}
			Node->SetArrayField(TEXT("outputs"), OutsArr);
		}
		...
		if (UMaterialExpressionScalarParameter* SP = Cast<UMaterialExpressionScalarParameter>(Expr))
		{
			Node->SetStringField(TEXT("parameter_name"), SP->ParameterName.ToString());
			Node->SetNumberField(TEXT("value"), SP->DefaultValue);
			Node->SetStringField(TEXT("group"), SP->Group.ToString());
		}
		else if (UMaterialExpressionVectorParameter* VP = Cast<UMaterialExpressionVectorParameter>(Expr))
		{
			Node->SetStringField(TEXT("parameter_name"), VP->ParameterName.ToString());
			TArray<TSharedPtr<FJsonValue>> C;
			C.Add(MakeShared<FJsonValueNumber>(VP->DefaultValue.R));
			C.Add(MakeShared<FJsonValueNumber>(VP->DefaultValue.G));
			C.Add(MakeShared<FJsonValueNumber>(VP->DefaultValue.B));
			C.Add(MakeShared<FJsonValueNumber>(VP->DefaultValue.A));
			Node->SetArrayField(TEXT("value"), C);
			Node->SetStringField(TEXT("group"), VP->Group.ToString());
		}
		else if (UMaterialExpressionTextureSampleParameter* TP = Cast<UMaterialExpressionTextureSampleParameter>(Expr))
		{
			Node->SetStringField(TEXT("parameter_name"), TP->ParameterName.ToString());
			if (TP->Texture) { Node->SetStringField(TEXT("texture"), TP->Texture->GetPathName()); }
			Node->SetStringField(TEXT("group"), TP->Group.ToString());
		}
		// Texture samples (non-parameter)
		else if (UMaterialExpressionTextureBase* TB = Cast<UMaterialExpressionTextureBase>(Expr))
		{
			if (TB->Texture) { Node->SetStringField(TEXT("texture"), TB->Texture->GetPathName()); }
		}
		// Custom HLSL
		else if (UMaterialExpressionCustom* CE = Cast<UMaterialExpressionCustom>(Expr))
		{
			Node->SetStringField(TEXT("code"), CE->Code);
			Node->SetStringField(TEXT("output_type"),
				StaticEnum<ECustomMaterialOutputType>()->GetNameStringByValue(static_cast<int64>(CE->OutputType)));
			if (!CE->Description.IsEmpty())
			{
				Node->SetStringField(TEXT("function_name"), CE->Description);
			}
			TArray<TSharedPtr<FJsonValue>> CInputs;
			for (const FCustomInput& CI : CE->Inputs)
			{
				TSharedPtr<FJsonObject> CIO = MakeShared<FJsonObject>();
				CIO->SetStringField(TEXT("name"), CI.InputName.ToString());
				CInputs.Add(MakeShared<FJsonValueObject>(CIO));
			}
			Node->SetArrayField(TEXT("custom_inputs"), CInputs);
			if (CE->AdditionalOutputs.Num() > 0)
			{
				TArray<TSharedPtr<FJsonValue>> COutputs;
				for (const FCustomOutput& CO : CE->AdditionalOutputs)
				{
					TSharedPtr<FJsonObject> COO = MakeShared<FJsonObject>();
					COO->SetStringField(TEXT("name"), CO.OutputName.ToString());
					COO->SetStringField(TEXT("type"),
						StaticEnum<ECustomMaterialOutputType>()->GetNameStringByValue(static_cast<int64>(CO.OutputType)));
					COutputs.Add(MakeShared<FJsonValueObject>(COO));
				}
				Node->SetArrayField(TEXT("additional_outputs"), COutputs);
			}
			if (CE->IncludeFilePaths.Num() > 0)
			{
				TArray<TSharedPtr<FJsonValue>> Includes;
				for (const FString& Inc : CE->IncludeFilePaths)
				{
					Includes.Add(MakeShared<FJsonValueString>(Inc));
				}
				Node->SetArrayField(TEXT("include_files"), Includes);
			}
		}
		// MaterialFunctionCall
		else if (UMaterialExpressionMaterialFunctionCall* FC = Cast<UMaterialExpressionMaterialFunctionCall>(Expr))
		{
			if (FC->MaterialFunction)
			{
				Node->SetStringField(TEXT("function_path"), FC->MaterialFunction->GetPathName());
				Node->SetStringField(TEXT("function_name"), FC->MaterialFunction->GetName());
			}
		}
		// ComponentMask
		else if (UMaterialExpressionComponentMask* CM = Cast<UMaterialExpressionComponentMask>(Expr))
		{
			Node->SetBoolField(TEXT("r"), CM->R != 0);
			Node->SetBoolField(TEXT("g"), CM->G != 0);
			Node->SetBoolField(TEXT("b"), CM->B != 0);
			Node->SetBoolField(TEXT("a"), CM->A != 0);
		}
		// Constants
		else if (UMaterialExpressionConstant* C1 = Cast<UMaterialExpressionConstant>(Expr))
		{
			Node->SetNumberField(TEXT("value"), C1->R);
		}
		else if (UMaterialExpressionConstant3Vector* C3 = Cast<UMaterialExpressionConstant3Vector>(Expr))
		{
			TArray<TSharedPtr<FJsonValue>> C;
			C.Add(MakeShared<FJsonValueNumber>(C3->Constant.R));
			C.Add(MakeShared<FJsonValueNumber>(C3->Constant.G));
			C.Add(MakeShared<FJsonValueNumber>(C3->Constant.B));
			Node->SetArrayField(TEXT("value"), C);
		}
		// TextureCoordinate
		else if (UMaterialExpressionTextureCoordinate* TC = Cast<UMaterialExpressionTextureCoordinate>(Expr))
		{
			Node->SetNumberField(TEXT("coordinate_index"), TC->CoordinateIndex);
			Node->SetNumberField(TEXT("u_tiling"), TC->UTiling);
			Node->SetNumberField(TEXT("v_tiling"), TC->VTiling);
		}
		// Named reroutes
		else if (UMaterialExpressionNamedRerouteDeclaration* NRD = Cast<UMaterialExpressionNamedRerouteDeclaration>(Expr))
		{
			Node->SetStringField(TEXT("reroute_name"), NRD->Name.ToString());
			Node->SetStringField(TEXT("reroute_guid"), NRD->VariableGuid.ToString());
		}
		else if (UMaterialExpressionNamedRerouteUsage* NRU = Cast<UMaterialExpressionNamedRerouteUsage>(Expr))
		{
			Node->SetStringField(TEXT("declaration_guid"), NRU->DeclarationGuid.ToString());
			if (NRU->Declaration)
			{
				Node->SetStringField(TEXT("declaration_name"), NRU->Declaration->Name.ToString());
			}
		}
		// Comment
		else if (UMaterialExpressionComment* Comment = Cast<UMaterialExpressionComment>(Expr))
		{
			Node->SetStringField(TEXT("comment_text"), Comment->Text);
		...
		{
			Node->SetBoolField(TEXT("default_value"), SS->DefaultValue != 0);
		}
		// FunctionInput/Output (when inside a function)
		else if (UMaterialExpressionFunctionInput* FI = Cast<UMaterialExpressionFunctionInput>(Expr))
		{
			Node->SetStringField(TEXT("input_name"), FI->InputName.ToString());
			Node->SetStringField(TEXT("input_type"),
				StaticEnum<EFunctionInputType>()->GetNameStringByValue(static_cast<int64>(FI->InputType)));
			Node->SetNumberField(TEXT("sort_priority"), FI->SortPriority);
		}
		else if (UMaterialExpressionFunctionOutput* FO = Cast<UMaterialExpressionFunctionOutput>(Expr))
		{
			Node->SetStringField(TEXT("output_name"), FO->OutputName.ToString());
			Node->SetNumberField(TEXT("sort_priority"), FO->SortPriority);
		}
		...
		for (int32 i = 0; i < Inputs.Num(); ++i)
		{
			...
			if (Input->Expression)
			{
				TSharedPtr<FJsonObject> Conn = SerializeInputConnection(Input, Expr, i);
	...
	if (Material)
	{
		...
		if (EdOnly)
		{
			...
			auto AddOutput = [&](const TCHAR* PropName, FExpressionInput& Input)
			{
				if (Input.Expression)
				{
					TSharedPtr<FJsonObject> Out = MakeShared<FJsonObject>();
					Out->SetStringField(TEXT("property"), PropName);
					Out->SetStringField(TEXT("from_node"), GetExprNodeName(Input.Expression));
					Out->SetNumberField(TEXT("from_output_index"), Input.OutputIndex);
					if (Input.Expression->Outputs.IsValidIndex(Input.OutputIndex) &&
						!Input.Expression->Outputs[Input.OutputIndex].OutputName.IsNone())
					{
						Out->SetStringField(TEXT("from_output_name"),
							Input.Expression->Outputs[Input.OutputIndex].OutputName.ToString());
					}
					OutputsArr.Add(MakeShared<FJsonValueObject>(Out));
				}
			};

			AddOutput(TEXT("BaseColor"), EdOnly->BaseColor);
			AddOutput(TEXT("Metallic"), EdOnly->Metallic);
			AddOutput(TEXT("Specular"), EdOnly->Specular);
			AddOutput(TEXT("Roughness"), EdOnly->Roughness);
			AddOutput(TEXT("Anisotropy"), EdOnly->Anisotropy);
			AddOutput(TEXT("Normal"), EdOnly->Normal);
			AddOutput(TEXT("Tangent"), EdOnly->Tangent);
			AddOutput(TEXT("EmissiveColor"), EdOnly->EmissiveColor);
			AddOutput(TEXT("Opacity"), EdOnly->Opacity);
			AddOutput(TEXT("OpacityMask"), EdOnly->OpacityMask);
			AddOutput(TEXT("WorldPositionOffset"), EdOnly->WorldPositionOffset);
			AddOutput(TEXT("Displacement"), EdOnly->Displacement);
			AddOutput(TEXT("SubsurfaceColor"), EdOnly->SubsurfaceColor);
			AddOutput(TEXT("ClearCoat"), EdOnly->ClearCoat);
			AddOutput(TEXT("ClearCoatRoughness"), EdOnly->ClearCoatRoughness);
			AddOutput(TEXT("AmbientOcclusion"), EdOnly->AmbientOcclusion);
			AddOutput(TEXT("Refraction"), EdOnly->Refraction);
			AddOutput(TEXT("PixelDepthOffset"), EdOnly->PixelDepthOffset);
			AddOutput(TEXT("SurfaceThickness"), EdOnly->SurfaceThickness);
			AddOutput(TEXT("FrontMaterial"), EdOnly->FrontMaterial);
			AddOutput(TEXT("MaterialAttributes"), EdOnly->MaterialAttributes);

			Result->SetArrayField(TEXT("material_outputs"), OutputsArr);
		}
	}

	// ---- COMMENTS (from EditorComments collection) ----
	if (Material)
	{
		if (UMaterialEditorOnlyData* EdOnly = Material->GetEditorOnlyData())
		{
			TArray<TSharedPtr<FJsonValue>> CommentsArr;
			for (const TObjectPtr<UMaterialExpressionComment>& CommentPtr : EdOnly->ExpressionCollection.EditorComments)
			{
				if (!CommentPtr) { continue; }
				TSharedPtr<FJsonObject> CObj = MakeShared<FJsonObject>();
				CObj->SetStringField(TEXT("text"), CommentPtr->Text);
				CObj->SetNumberField(TEXT("x"), CommentPtr->MaterialExpressionEditorX);
				CObj->SetNumberField(TEXT("y"), CommentPtr->MaterialExpressionEditorY);
				CObj->SetNumberField(TEXT("size_x"), CommentPtr->SizeX);
				CObj->SetNumberField(TEXT("size_y"), CommentPtr->SizeY);
				CommentsArr.Add(MakeShared<FJsonValueObject>(CObj));
			}
			if (CommentsArr.Num() > 0)
			{
	...
	return Result;
#else
	return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Requires editor build"));
#endif
}
...
TSharedPtr<FJsonObject> FUnrealMCPMaterialCommands::HandleAddFunctionNode(const TSharedPtr<FJsonObject>& Params)
{
	...
	// Handle MaterialFunctionCall — assign target function and regenerate pins.
	// Required for nested MaterialFunctionCall nodes inside a MaterialFunction so
	// connect_function_nodes can resolve named pins on the call.
	if (UMaterialExpressionMaterialFunctionCall* FuncCall = Cast<UMaterialExpressionMaterialFunctionCall>(NewExpression))
	{
		FString FuncPath;
		bool bHaveFuncPath = Params->TryGetStringField(TEXT("function_path"), FuncPath);
...
TSharedPtr<FJsonObject> FUnrealMCPMaterialCommands::HandleAddMaterialFunctionCall(const TSharedPtr<FJsonObject>& Params)
{
	...
	// Auto-detect: try as UMaterial first, then as UMaterialFunction so callers can
	// add MaterialFunctionCall nodes inside another material function.
	UMaterial* Material = nullptr;
	UMaterialFunction* HostFunction = nullptr;
	{
		...
		if (!Material)
		{
			HostFunction = LoadMaterialFunction(MaterialName, TryPath);
			if (!HostFunction)
			{
				return FUnrealMCPCommonUtils::CreateErrorResponse(
					FString::Printf(TEXT("Material or MaterialFunction not found: %s"), *MaterialName));
	...
	UMaterialExpressionMaterialFunctionCall* FuncCallExpr = Cast<UMaterialExpressionMaterialFunctionCall>(
		UMaterialEditingLibrary::CreateMaterialExpressionEx(
			Material,       // nullptr when host is a function
			HostFunction,   // nullptr when host is a material
...
void FUnrealMCPMaterialCommands::RegisterCommands(FMCPCommandRegistry& Registry)
{
	...
	Registry.RegisterCommand(TEXT("compile_material_detailed"),
		[this](const TSharedPtr<FJsonObject>& P) { return HandleCommand(TEXT("compile_material_detailed"), P); });
	...
	Registry.RegisterCommand(TEXT("get_material_info"),
		[this](const TSharedPtr<FJsonObject>& P) { return HandleCommand(TEXT("get_material_info"), P); });
