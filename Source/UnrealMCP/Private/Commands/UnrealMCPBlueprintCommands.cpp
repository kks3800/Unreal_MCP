...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
    ...
    else if (CommandType == TEXT("compile_blueprint_detailed"))
    {
        return HandleCompileBlueprintDetailed(Params);
    }
    // Editor
    else if (CommandType == TEXT("open_blueprint"))
    {
        return HandleOpenBlueprint(Params);
    }
    // Batch Operations
    else if (CommandType == TEXT("begin_blueprint_edit"))
    {
        return HandleBeginBlueprintEdit(Params);
    }
    else if (CommandType == TEXT("end_blueprint_edit"))
    {
        return HandleEndBlueprintEdit(Params);
    }
    else if (CommandType == TEXT("execute_blueprint_batch"))
    {
        return HandleExecuteBlueprintBatch(Params);
    }
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleCreateBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (!ParentClass.IsEmpty())
    {
        ...
        // If a full path was provided (e.g. "/Script/Module.AClassName"), try it directly
        if (ParentClass.StartsWith(TEXT("/")))
        {
            FoundClass = LoadClass<AActor>(nullptr, *ParentClass);
        }
        else
        {
            ...
            else
            {
                ...
                {
                    const FString ClassPath = ModulePath + ClassName;
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleCompileBlueprintDetailed(const TSharedPtr<FJsonObject>& Params)
{
#if WITH_EDITOR
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    ...
    // Compile status
    bool bCompileSuccess = (Blueprint->Status == BS_UpToDate);
    Data->SetBoolField(TEXT("compiled"), bCompileSuccess);
    ...
    for (const TSharedRef<FTokenizedMessage>& Message : ResultsLog.Messages)
    {
        ...
        // Try to extract node GUID from message tokens
        for (const TSharedRef<IMessageToken>& Token : Message->GetMessageTokens())
        {
            if (Token->GetType() == EMessageToken::Object)
            {
                FString TokenStr = Token->ToText().ToString();
                MsgObj->SetStringField(TEXT("object_ref"), TokenStr);
            }
        }
        ...
        switch (Message->GetSeverity())
        {
            ...
            case EMessageSeverity::PerformanceWarning:
                ...
                break;
            default:
                NotesArray.Add(MakeShared<FJsonValueObject>(MsgObj));
                break;
        }
    }
    ...
    Data->SetArrayField(TEXT("notes"), NotesArray);
    Data->SetNumberField(TEXT("error_count"), ErrorsArray.Num());
    Data->SetNumberField(TEXT("warning_count"), WarningsArray.Num());

    return Data;
#else
    return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Detailed compilation is only available in editor builds"));
#endif
}
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleOpenBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	FString BlueprintName;
	if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
	{
	...
	TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
	ResultObj->SetStringField(TEXT("opened"), BlueprintName);
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleBeginBlueprintEdit(const TSharedPtr<FJsonObject>& Params)
{
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    {
    ...
    if (FMCPBlueprintContext::Get().BeginEdit(BlueprintName))
    {
        TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
        ResultObj->SetBoolField(TEXT("success"), true);
        ResultObj->SetStringField(TEXT("message"), FString::Printf(TEXT("Started batch editing %s"), *BlueprintName));
        return ResultObj;
    }

    return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Failed to begin editing %s"), *BlueprintName));
}

TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleEndBlueprintEdit(const TSharedPtr<FJsonObject>& Params)
{
    if (!FMCPBlueprintContext::Get().IsEditing())
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("No Blueprint is currently being edited"));
    }
    ...
    FString AutoLayoutGraph;
    Params->TryGetStringField(TEXT("auto_layout"), AutoLayoutGraph);

    if (FMCPBlueprintContext::Get().EndEdit(bAutoCompile, bAutoSave, AutoLayoutGraph))
    {
        TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
        ResultObj->SetBoolField(TEXT("success"), true);
        ResultObj->SetStringField(TEXT("message"), TEXT("Blueprint compiled and saved"));
        return ResultObj;
    }

    return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Failed to end Blueprint edit"));
}
 ...
 */
static FString ResolveBatchReference(const FString& Value, const TArray<TSharedPtr<FJsonValue>>& PreviousResults)
{
    ...
    // Match patterns like $0.node_id, $1.variable_name, $2, etc.
    ...
    while (Matcher.FindNext())
    {
        ...
        if (Index >= 0 && Index < PreviousResults.Num())
        {
            ...
            if (PreviousResults[Index]->TryGetObject(ResultObj) && ResultObj && ResultObj->IsValid())
            {
                ...
                {
                    // $N without field - try node_id as default
                    FString NodeId;
                    if ((*ResultObj)->TryGetStringField(TEXT("node_id"), NodeId))
                    {
                        Result = Result.Replace(*FullMatch, *NodeId);
                    }
                }
                else
                {
...
static void ResolveReferencesInObject(TSharedPtr<FJsonObject>& Obj, const TArray<TSharedPtr<FJsonValue>>& PreviousResults)
{
    TArray<FString> Keys;
    ...
    for (const FString& Key : Keys)
    {
        ...
        {
            FString Resolved = ResolveBatchReference(Val->AsString(), PreviousResults);
        ...
        else if (Val->Type == EJson::Object)
        {
            ...
            ResolveReferencesInObject(SubObj, PreviousResults);
        }
    }
}

TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleExecuteBlueprintBatch(const TSharedPtr<FJsonObject>& Params)
{
    ...
    for (int32 OpIndex = 0; OpIndex < OperationsArray->Num(); ++OpIndex)
    {
        ...
        // Build params: copy all fields except 'op', inject blueprint_name
        ...
        OpParams->SetStringField(TEXT("blueprint_name"), BlueprintName);

        // Resolve $N references using previous results
        ResolveReferencesInObject(OpParams, Results);

        // Route through the command registry (which has all commands registered)
        TSharedPtr<FJsonObject> OpResult;
        auto& Registry = FMCPCommandRegistry::Get();
        ...
        {
            SuccessCount++;
            FMCPBlueprintContext::Get().MarkDirty();
        }
        else
        {
            FailCount++;
        ...
        FMCPBlueprintContext::Get().LogOperation(
            FString::Printf(TEXT("[%d] %s"), OpIndex, *OpCommand), bOpSuccess);
...
void FUnrealMCPBlueprintCommands::RegisterCommands(FMCPCommandRegistry& Registry)
{
	Registry.RegisterCommand(TEXT("create_blueprint"),
		[this](const TSharedPtr<FJsonObject>& P) { return HandleCommand(TEXT("create_blueprint"), P); });
	Registry.RegisterCommand(TEXT("add_component_to_blueprint"),
		[this](const TSharedPtr<FJsonObject>& P) { return HandleCommand(TEXT("add_component_to_blueprint"), P); });
	Registry.RegisterCommand(TEXT("set_component_property"),
		[this](const TSharedPtr<FJsonObject>& P) { return HandleCommand(TEXT("set_component_property"), P); });
	Registry.RegisterCommand(TEXT("set_physics_properties"),
		[this](const TSharedPtr<FJsonObject>& P) { return HandleCommand(TEXT("set_physics_properties"), P); });
	Registry.RegisterCommand(TEXT("compile_blueprint"),
		[this](const TSharedPtr<FJsonObject>& P) { return HandleCommand(TEXT("compile_blueprint"), P); });
	Registry.RegisterCommand(TEXT("set_blueprint_property"),
		[this](const TSharedPtr<FJsonObject>& P) { return HandleCommand(TEXT("set_blueprint_property"), P); });
	Registry.RegisterCommand(TEXT("set_static_mesh_properties"),
		[this](const TSharedPtr<FJsonObject>& P) { return HandleCommand(TEXT("set_static_mesh_properties"), P); });
	Registry.RegisterCommand(TEXT("set_pawn_properties"),
		[this](const TSharedPtr<FJsonObject>& P) { return HandleCommand(TEXT("set_pawn_properties"), P); });
	Registry.RegisterCommand(TEXT("compile_blueprint_detailed"),
		[this](const TSharedPtr<FJsonObject>& P) { return HandleCommand(TEXT("compile_blueprint_detailed"), P); });
	// Editor
	Registry.RegisterCommand(TEXT("open_blueprint"),
		[this](const TSharedPtr<FJsonObject>& P) { return HandleCommand(TEXT("open_blueprint"), P); });
	// Batch Operations
	Registry.RegisterCommand(TEXT("begin_blueprint_edit"),
		[this](const TSharedPtr<FJsonObject>& P) { return HandleCommand(TEXT("begin_blueprint_edit"), P); });
	Registry.RegisterCommand(TEXT("end_blueprint_edit"),
		[this](const TSharedPtr<FJsonObject>& P) { return HandleCommand(TEXT("end_blueprint_edit"), P); });
	Registry.RegisterCommand(TEXT("execute_blueprint_batch"),
		[this](const TSharedPtr<FJsonObject>& P) { return HandleCommand(TEXT("execute_blueprint_batch"), P); });
}