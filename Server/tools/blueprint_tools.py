...
def register_blueprint_tools(mcp: FastMCP):
    ...
    @mcp.tool()
    def compile_blueprint_detailed(
        ctx: Context,
        blueprint_name: str
    ) -> Dict[str, Any]:
        """
        Compile a Blueprint and return detailed results including errors, warnings, and notes.

        Args:
            blueprint_name: Name of the target Blueprint

        Returns:
            Dict with compiled status, error_count, warning_count, and arrays of errors/warnings/notes
        """
        from unreal_mcp_server import get_unreal_connection
        ...
        try:
            ...
            response = unreal.send_command("compile_blueprint_detailed", {
                "blueprint_name": blueprint_name
            })
            ...
            logger.error(f"Error in compile_blueprint_detailed: {e}")
    ...
    def open_blueprint(
        ...
        """
        Open a Blueprint in the Unreal Editor for live viewing.

        Opens the Blueprint editor window so you can see changes as they
        are made programmatically. Call this before build_blueprint_graph
        or execute_blueprint_batch to watch edits appear live.

        Args:
            blueprint_name: Name or path of the Blueprint to open

        Returns:
            Dict with the opened Blueprint name
        """
        from unreal_mcp_server import get_unreal_connection
        ...
        try:
            ...
            response = unreal.send_command("open_blueprint", {
                "blueprint_name": blueprint_name
            })
            ...
            logger.error(f"Error in open_blueprint: {e}")
